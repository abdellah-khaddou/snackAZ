<aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        <ul class="sidebar-menu" data-widget="tree">
            <li>
                <a href="{{ url('/admin') }}"><i class="fa fa-th"></i><span>@lang('site.dashboard')</span>
                </a>
            </li>
            @php
              $models =config('global.models');
              $classes=config('global.classes');
            @endphp

            @foreach($models as $index=>$table )
                @if(auth()->guard('admin')->user()->hasPermission('read-'.$table))
                    <li>
                        <a href="{{url('/admin/'.$table)}}"><i class="{{$classes[$index]}}"></i> <span> @lang('site.'.$table)</span>
                        </a>
                    </li>
                @endif
            @endforeach
