...
class CatagoryDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->escapeColumns('name')
            ->addColumn('name',function ($catagory){
                return '<a href="'.route("admin.catagories.show",$catagory->id).'">'.$catagory->name.'</a>';
            })
            ->addColumn('nb_produits', function ($catagory){
               return count($catagory->produits);
            })->addColumn('action', function ($catagory){
                return view('admin.catagories.actions.action',compact('catagory'))->render();
            });
    }

    public function query(Catagory $model)
    {
        return Catagory::query();
    }
    ...
    public function html()
    {
        ...
        return $this->builder()
            ->setTableId('catagoriedatatable-table')
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::computed('name')
                ...
                ->orderable(true)
                ->searchable(true),
            Column::computed('nb_produits')
                ->addClass('text-center')
                ->title(trans(("site.nb_produits")))
                ->orderable(true)
                ->searchable(true),
